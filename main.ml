	...
	let rx=ref 0 and ry=ref 0 and rd=ref d in (** Nowe wartosci koordynatow (x,y) oraz opoznienia (d). *)
		...
		(
			...
			let collision=board#collided (x,y) in (** Jesli od bloku planszy. *)
				...
				|2->ry:=-y (** Jesli od gornej lub dowlnej sciany, zmiana wspolrzednej y. *)
	...
	let rec navi_aux i=
		...
		|true-> (** Jesli wcisinieto przycisk - wykonanie akcji. *)
			...
			(
				...
				|1->menu#drawInstructions;instructionsNavigation();navi_aux (wait_next_event [Mouse_motion;Button_down]) (** Rysowanie instrukcji. *)
				|2-> (** Uruchamianie nowej gry. *)
				...
				|3->locker:=false;background#draw;board#drawCollection;plateNavigation();navi_aux (wait_next_event [Mouse_motion;Button_down]) (** Wlaczenie pauzy. *)
